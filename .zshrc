#debpkg zsh-syntax-highlighting 
#debpkg auto-suggestions zsh-autosuggestions
...
setopt promptsubst         # enable command substitution ina prompt
...
if [ "$color_prompt" = yes ]; then
    PROMPT=$'%F{%(#.pink.pink)}┌${debian_chroot:+($debian_chroot)──}$(venv_info)(%B%F{%(#.red.blue)}%n%(#.💀.%F{pink}@%F{blue})%m%b%F{%(#.blue.pink)})%F{pink}─%F{pink}[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.pink)}]\n└%B%(#.%F{blue}#.%F{blue}$)%b%F{reset} '
...
if [ -x /usr/bin/dircolors ]; then
    ...
    alias syu='sudo apt full-upgrade -y'
    alias gc='git clone'
